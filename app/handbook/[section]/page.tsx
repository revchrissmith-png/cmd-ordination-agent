...
import { WIKI_SECTIONS, getSectionBySlug, getAdjacentSections, type ContentBlock, type WikiSection } from '../content'
...
export default function HandbookSection() {
  ...
  useEffect(() => {
    ...
    if (section.subsections.length > 0) setActiveSubId(section.subsections[0].id)
  }, [slug])
              ...
              {prev ? (
                <Link href={`/handbook/${prev.slug}`}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm font-black text-slate-700 group-hover:text-blue-700 transition-colors truncate">{prev.title}</p>
                  </div>
                </Link>
              ...
              {next ? (
                <Link href={`/handbook/${next.slug}`}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm font-black text-slate-700 group-hover:text-blue-700 transition-colors truncate">{next.title}</p>
                  </div>
                  <span className="text-slate-300 group-hover:text-blue-500 font-bold transition-colors">→</span>
                </Link>
              ) : <div className="flex-1" />}
