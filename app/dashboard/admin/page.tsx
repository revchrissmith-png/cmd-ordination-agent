      ...
      <div className="max-w-5xl mx-auto">
        ...
        {activeTab === 'council' && (
          <div className="space-y-6">
                    ...
                    {councilMembers.map(m => (
                      <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-8 py-4 text-right">
                          <button onClick={() => handleRemoveCouncil(m.id, `${m.first_name} ${m.last_name}`)} className="text-red-400 hover:text-red-600 font-bold text-sm transition-colors">Remove</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <p className="text-xs text-slate-400 font-medium px-1">Removing a council member changes their role but does not delete their account. Existing grading assignments remain until manually reassigned.</p>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8">
              ...
              <form onSubmit={handleAddCohort} className="space-y-4">
                ...
                <div>
                  <label className={labelClass}>Cohort Name <span className="normal-case font-medium text-slate-400">(optional — auto-generated if blank)</span></label>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {cohorts.map(c => (
                      <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-8 py-5">
                          <span className="px-3 py-1 bg-purple-50 text-purple-700 rounded-full text-xs font-bold">{topicLabel(c.sermon_topic)}</span>
                        </td>
                        <td className="px-8 py-5">
                          <div className="flex flex-wrap gap-1">
                            {TOPICS.filter(t => t.value !== c.sermon_topic).map(t => (
                              <span key={t.value} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full text-xs font-medium">{t.label}</span>
                            ))}
                          </div>
                        </td>
                      </tr>
                ...
                <form onSubmit={handleAddCandidate} className="space-y-4">
                  ...
                  <div>
                    ...
                    <select className={inputClass} value={newCandidateCohort} onChange={e => setNewCandidateCohort(e.target.value)} required>
                      ...
                      {cohorts.map(c => (
                        <option key={c.id} value={c.id}>{c.name} — Sermons: {topicLabel(c.sermon_topic)}</option>
                      ))}
                    </select>
                  </div>
                    ...
                    {candidates.map(person => (
                      <tr key={person.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-8 py-5 text-right">
                          <button className="text-blue-600 font-black hover:text-blue-800 transition-colors text-sm">Manage →</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
