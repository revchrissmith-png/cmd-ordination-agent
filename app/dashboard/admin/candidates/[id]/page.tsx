...
type Requirement = {
  id: string
  status: string
  template: { type: string; topic: string | null; title: string; book_category: string | null; sermon_question_index: number | null; display_order: number }
  assignment: { id: string; council_member_id: string; notes: string | null; council_member: { first_name: string; last_name: string; email: string } } | null
}

const TYPE_LABELS: Record<string, string> = { book_report: 'Book Report', paper: 'Theological Paper', sermon: 'Sermon' }
const STATUS_COLORS: Record<string, string> = { not_started: 'bg-slate-100 text-slate-500', in_progress: 'bg-amber-100 text-amber-700', submitted: 'bg-blue-100 text-blue-700', graded: 'bg-green-100 text-green-700', resubmit: 'bg-red-100 text-red-700' }
...
export default function CandidateManagePage() {
  ...
  function flash(text: string, type: 'success' | 'error') { setMessage({ text, type }); setTimeout(() => setMessage({ text: '', type: '' }), 5000) }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 font-mono text-blue-600">Loading...</div>
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 md:p-10">
      <div className="max-w-4xl mx-auto">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {reqs.map(req => (
                      <tr key={req.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4"><span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${STATUS_COLORS[req.status] ?? 'bg-slate-100 text-slate-500'}`}>{(req.status ?? 'not_started').replace('_', ' ')}</span></td>
                        <td className="px-6 py-4">
                          {req.assignment?.council_member
                            ? <div><div className="font-bold text-slate-800 text-sm">{req.assignment.council_member.first_name} {req.assignment.council_member.last_name}</div><div className="text-xs text-slate-400">{req.assignment.council_member.email}</div></div>
                            : <span className="text-slate-400 text-sm italic">Unassigned</span>}
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-3">
                            <button onClick={() => openAssign(req)} className="text-blue-600 hover:text-blue-800 font-bold text-sm">{req.assignment ? 'Reassign' : 'Assign'}</button>
                            {req.assignment && <button onClick={() => removeAssignment(req)} className="text-red-400 hover:text-red-600 font-bold text-sm">Remove</button>}
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        {requirements.length === 0 && <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-12 text-center"><p className="text-slate-400">No requirements found for this candidate.</p></div>}
      </div>
      ...
      {assigningReq && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8">
            ...
            <p className="text-slate-500 text-sm font-medium mb-6">{assigningReq.template?.title}</p>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-1.5">COI / Notes (optional)</label>
                <textarea className={`${inputClass} resize-none h-24`} value={assignNotes} onChange={e => setAssignNotes(e.target.value)} placeholder="Note any conflict of interest..." />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setAssigningReq(null)} className="flex-1 py-3 rounded-xl font-bold text-slate-500 bg-slate-100 hover:bg-slate-200">Cancel</button>
              <button onClick={saveAssignment} disabled={!selectedCouncil || isSaving} className="flex-1 py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300">
                {isSaving ? 'Saving...' : assigningReq.assignment ? 'Update' : 'Assign'}
              </button>
            </div>
          </div>
        </div>
