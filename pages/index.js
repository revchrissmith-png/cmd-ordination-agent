...
export default function OrdinationAgent() {
  ...
  // --- VIEW 1: SIGN IN (Only shows if not logged in) ---
  if (!user) {
    return (
  ...
  // --- VIEW 2: CHAT (Only shows if logged in) ---
  return (
    <div style={{ backgroundColor: colors.cloudGray, minHeight: '100vh', fontFamily: 'Arial, sans-serif' }}>
      ...
      <header style={{ backgroundColor: colors.deepSea, color: colors.white, padding: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `4px solid ${colors.allianceBlue}` }}>
        ...
        <button onClick={() => supabase.auth.signOut()} style={{ background: 'transparent', border: '1px solid white', color: 'white', padding: '0.4rem 0.7rem', borderRadius: '4px', fontSize: '0.7rem', cursor: 'pointer' }}>LOGOUT</button>
      </header>
      ...
      <main style={{ maxWidth: '850px', margin: '1.5rem auto', padding: '0 0.5rem' }}>
        <div style={{ backgroundColor: colors.white, borderRadius: '4px', height: '75vh', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
          <div ref={scrollRef} style={{ flex: 1, padding: '1.2rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {messages.length === 0 && <p style={{ textAlign: 'center', color: colors.allianceBlue, marginTop: '2rem' }}>Welcome, {getUserName()}. What theological or policy topic should we practice today?</p>}
            {messages.map((msg, i) => (
              <div key={i} style={{ alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', backgroundColor: msg.role === 'user' ? colors.allianceBlue : colors.cloudGray, color: msg.role === 'user' ? colors.white : colors.charcoal, padding: '0.9rem 1.1rem', borderRadius: '8px', maxWidth: '88%', fontSize: '0.95rem', whiteSpace: 'pre-wrap' }}>
                {msg.content}
              </div>
            ))}
