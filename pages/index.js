      ...
      <main style={{ maxWidth: '850px', margin: '2rem auto', padding: '0 1rem' }}>
        <div style={{ backgroundColor: colors.white, borderRadius: '4px', height: '65vh', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
          <div ref={scrollRef} style={{ flex: 1, padding: '1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ 
                ...
                padding: '0.9rem 1.3rem', borderRadius: '8px', maxWidth: '85%', fontSize: '0.95rem',
                whiteSpace: 'pre-wrap' // This preserves the line breaks from the API
              }}>
                {msg.content}
              </div>
            ))}
          ...
          <form onSubmit={handleSendMessage} style={{ padding: '1.5rem', borderTop: `1px solid ${colors.cloudGray}`, display: 'flex', gap: '0.8rem' }}>
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your answer..." style={{ flex: 1, padding: '0.8rem', border: `1px solid ${colors.allianceBlue}`, borderRadius: '4px' }} />
        ...
        <p style={{ textAlign: 'center', color: '#999', fontSize: '0.65rem', marginTop: '1rem' }}>Build v1.7.0 | Clean Text Interface</p>
      </main>
    </div>
  );
}
